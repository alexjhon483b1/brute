...
int main(int argc, char *argv[]) {
    ...
    if (curl) {
        ...
        const char *post_data = "userId=3117698&password=964063&sessionId=Honours";
        ...
        headers = curl_slist_append(headers, "Host: app1.nu.edu.bd");
        ...
        headers = curl_slist_append(headers, "Origin: http://app1.nu.edu.bd");
        headers = curl_slist_append(headers, "DNT: 1");
        headers = curl_slist_append(headers, "Connection: keep-alive");
        headers = curl_slist_append(headers, "Referer: http://app55.nu.edu.bd/nu-web/applicantLogin.action");
        char cookie_header[100];  // Adjust the size accordingly
         snprintf(cookie_header, sizeof(cookie_header), "Cookie: %s", argv[2]);
